#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <string.h>
#include "ceebee/cpu.h"
#include "ceebee/common.h"
#include "ceebee/mmu.h"
#include "ceebee/termColors.h"

/* BOOT ROM */
static uint8_t BIOS[0x100] = {
	 };

/* Load bootstrap in to memory */
/* https://blog.ryanlevick.com/DMG-01/public/book/memory_map.html#a0x0000---0x00ff-boot-rom */
/* The boot rom bootstraps into the main cart. Located at 0x0000 to 0x00FF */
void mmu_load_boot_rom(MMU *mmu) {
  memcpy(mmu->BIOS, BIOS, sizeof(BIOS));
}

// Reads 16 bit value to addr
uint16_t readNN(CPU *cpu, uint16_t addr) {
  bool BIOS_finished = *cpu->mmu->finishedBIOS;
  if (!BIOS_finished && addr < 0x0100) {
    return *((uint16_t*)(cpu->mmu->BIOS + addr)); 
  }
  return *((uint16_t*)(cpu->mmu->ram + addr)); 
}

// Reads 8 bit value to addr
uint8_t readN(CPU *cpu, uint16_t addr) {
  bool BIOS_finished = *cpu->mmu->finishedBIOS;
  if (!BIOS_finished && addr < 0x0100) {
    return *((uint8_t*)(cpu->mmu->BIOS + addr)); 
  }
  return *((uint8_t*)(cpu->mmu->ram + addr)); 
}

// Writes 16 bit value to addr
void writeNN(CPU *cpu, uint16_t addr, uint16_t val) {
  bool BIOS_finished = *cpu->mmu->finishedBIOS;
  if (!BIOS_finished && addr < 0x0100) {
    *((uint16_t*)(cpu->mmu->BIOS + addr)) = val; 
  }
  *((uint16_t*)(cpu->mmu->ram + addr)) = val;
}

// Writes 8 bit value to addr
void writeN(CPU *cpu, uint16_t addr, uint8_t val) {
  bool BIOS_finished = *cpu->mmu->finishedBIOS;
  if (!BIOS_finished && addr < 0x0100) {
    *((uint8_t*)(cpu->mmu->BIOS + addr)) = val; 
  }
  *((uint8_t*)(cpu->mmu->ram + addr)) = val;
}

void loadCart(CPU *cpu, char const *cartPath, unsigned int* cartSize) {
  FILE *fp;
  uint8_t *buffer;
  long cartLength;
  
  fp = fopen(cartPath, "rb");
  if (fp == NULL) {
    panic(cpu, RED "Could not load cart" RESET);
  }
  fseek(fp, 0, SEEK_END);
  cartLength = ftell(fp);
  *cartSize = cartLength;
  rewind(fp);

  buffer = (uint8_t*)malloc(cartLength * sizeof(uint8_t));
  fread(buffer, cartLength, 1, fp);
  memcpy(cpu->mmu->ram, buffer, cartLength * sizeof(uint8_t));
  free(buffer);

  fclose(fp);
}
